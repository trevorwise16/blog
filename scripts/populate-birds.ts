...
async function populateBirds() {
  ...
  try {
    ...
    const response = await fetch('https://api.ebird.org/v2/ref/taxonomy/ebird?fmt=json&locale=en')
    ...
    for (let i = 0; i < birdsToInsert.length; i += batchSize) {
      ...
      await db.insert(birds).values(batch).onConflictDoUpdate({
      ...
      console.log(`Inserted batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(birdsToInsert.length / batchSize)}`)
    }
    
    console.log(`✅ Successfully populated ${birdsToInsert.length} bird species!`)
